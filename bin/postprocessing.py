...
def plot_grn(df_preprocessed, layout="graphviz_layout", show_conflicting=True,
             filename=None, highlight_genes=None, condition1='condition 1', condition2 = "condtition 2"):
    ...
    if highlight_genes:
        nx.draw_networkx_nodes(G, pos, node_size=1500, ax=ax, node_color=node_colors)
    else:
        nx.draw_networkx_nodes(G, pos, node_size=1500, ax=ax, node_color='lightgray')
    ...
    if show_conflicting:
        ...
        ax.legend([arrow1, arrow2, arrow3], ['Stronger in '+condition1, 'Stronger in '+condition2, 'Conflicting'],
                  handler_map={mpatches.FancyArrow: HandlerPatch(patch_func=make_legend_arrow), }, loc="upper right",
                  fontsize=14)
    else:
        ax.legend([arrow1, arrow2], ['Stronger in '+condition1, 'Stronger in '+condition2],
                  handler_map={mpatches.FancyArrow: HandlerPatch(patch_func=make_legend_arrow), }, loc="upper right",
                  fontsize=14)
